...
ld func(ll x) {
    return ld(b)*x + ld(a)/sqrt(x + 1);
}
void solve_with_precision_error()
{
    cin >> a >> b;
    ll x = pow(ld(a)/(2*b), ld(2)/3) - 1;         // x == long long as x = #operations 
    // trying all nearer values of x to get the minimum
    ld poss1 = func(x);
    ld poss2 = func(x + 1);
    cout << fixed << setprecision(15) << min(poss1, poss2);
}
