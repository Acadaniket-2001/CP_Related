...
void solve()
{
    ...
    // solving each half separately.
    vector<int> soln_1 = generate(newarr[0]);
    vector<int> soln_2 = generate(newarr[1]);

    // efficienlty merging the 2 soln(s) in the middle of soln. flow to get the required answer.
    sort(all(soln_1));
    sort(all(soln_2));
    ll ans = 0;
    for(auto e: soln_1) {
        ans += upper_bound(all(soln_2), x - e) - soln_2.begin();
    }
