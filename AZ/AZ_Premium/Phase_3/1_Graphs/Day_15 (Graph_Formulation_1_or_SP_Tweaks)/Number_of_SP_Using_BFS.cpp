...
vector<vector<int>> dis, num_path;
...
void bfs(state st) {
    ...
    num_path.assign(n, vector<int>(n, 0));

    queue<state> q;
    dis[st.ff][st.ss] = 0;
    num_path[st.ff][st.ss] = 1;
    q.push(st);
    ...
    while(!q.empty()) {
       ...
       f(i, 0, 3) {
            int x = node.ff + dx[i];
            int y = node.ss + dy[i];
            if(is_valid(x, y) and g[x][y] != '#') {

                // method 1: Works only in BFS() not in Dijkstra()
                if(dis[x][y] == INF) {
                    dis[x][y] = dis[node.ff][node.ss] + 1;
                    num_path[x][y] = num_path[node.ff][node.ss];
                    q.push({x, y});
                }
                else if(dis[x][y] == dis[node.ff][node.ss] + 1) {
                    num_path[x][y] += num_path[node.ff][node.ss];
                }
                ...
                //     num_path[x][y] += num_path[node.ff][node.ss];
                // }
                // else {        //(dis[x][y] < dis[node.ff][node.ss] + 1)
                //     // nothing
                // }
            }
        } 
