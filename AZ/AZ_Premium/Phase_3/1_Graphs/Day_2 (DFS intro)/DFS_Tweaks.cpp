...
void solve() {
    ...
    f(i, 1, sz(components) - 1) {
        cout << "Comp. " <<i << ": " << components[i] << endl;
    }
...
void solve() {
    ...
    f(i, 1, n) {
        if(!vis[i]) {
            dfs(i, color);  // using color = 1 and 2
        }
    }

    cout << is_bipartite << endl;
    if(is_bipartite) {
        cout << "No odd length cycle present" << endl;
    }
    else {
        cout << "Odd length cycle present" << endl;
    }
...
// follow up -> print the cycles

int n, m;
vector<vector<int>> g;
vector<int> vis;

bool is_cycle_present = 0;

void dfs(int node, int par) {
    vis[node] = 1;
    for(auto v: g[node]) {
        if(!vis[v]) {
            dfs(v, node);                 // dfs(v, 3^color);
        }
        else if(v != par){
            is_cycle_present = 1;
        }
...
void solve() {
    ...
    f(i, 1, n) {
        if(!vis[i]) {
            dfs(i, par);  // passing dummy parent node
        }
    }

    cout << is_cycle_present << endl;
}
