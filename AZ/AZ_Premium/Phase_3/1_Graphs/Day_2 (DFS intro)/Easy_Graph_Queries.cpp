...
vector<vector<int>> g;
vector<int> vis;         // stores [nn] belongs to which comp. using colors.
vector<int> SIZE;        // stores the size of component to which [nn] belongs.

int find_size_dfs(int nn, int color) {
...
void fill_size_dfs(int nn, int sz) {
    SIZE[nn] = sz;
    for(auto v: g[nn]) {
        if(SIZE[v] != sz) {
            fill_size_dfs(v, sz);
...
void solve()
{
    ...
    f(i, 1, n) {
        if(!vis[i]) {
            color++;
            int comp_size = find_size_dfs(i, color);
            fill_size_dfs(i, comp_size);
        }
    }
...
vector<int> cnt;
// vis[ ] declared above is used to store the id of each node: (nodes in same comp. have same id)
