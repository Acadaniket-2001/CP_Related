...
vector<state> neighbours(state node) {
    ...
    f(i, 0, 3) {
        ...
        if(x >= 0 and x < n and y >= 0 and y < m and g[x][y] == 0)  ans.pb({x, y});
    }
    return ans;
}
...
int bfs(state src) {
    ...
    // can also write another bfs on same src again to do some other work like filll_size_bfs();
}

void fill_size_bfs(state src, int size) {  

    queue<state> q;
    vis[src.ff][src.ss] = 2;
    ans[src.ff][src.ss] = size;
    q.push(src);
    ...
    f(i, 0, n - 1) {
        f(j, 0, m - 1) {
            if(g[i][j] == 0 and !vis[i][j]) {
                int size = bfs({i, j});
                if(size != 1)   fill_size_bfs({i, j}, size);
            }        
        }
    }
