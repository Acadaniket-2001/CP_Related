...
int rec(int l, int r) {       
    ...
    // Xition
    int ans = 1 + (l + 1 <= r ? rec(l + 1, r) : 0);              // ⚠️⚠️ always beware that we should not make rec() beyond [l, r]. even if it happens; handle it in Xition or in pruning   

    if(l + 1 <= r and arr[l + 1] == arr[l]) {
        int op1 = 1 + rec(l + 2, r);
        ans = min(ans, op1);
    }
    ...
    for(int j = l + 2; j <= r; j++) {
        if(arr[j] == arr[l])
            ans = min(ans, rec(l + 1, j - 1) + rec(j + 1, r));
    }
