...
/*
Problem: https://www.geeksforgeeks.org/problems/matrix-chain-multiplication0303/1?itm_source=geeksforgeeks&itm_medium=article&itm_campaign=practice_card
*/
...
int rec(int l, int r) {
    ...
    int ans = INF;
    f(mid, l, r - 1) {
        if(rec(l, mid) + rec(mid + 1, r) + arr[l-1]*arr[mid]*arr[r] < ans) {
            ans = rec(l, mid) + rec(mid + 1, r) + arr[l-1]*arr[mid]*arr[r];
            back[l][r] = mid;
        }
    }
...
// ⭐⭐ storing th no. of brackets before and after each element, for printing / General Technique for printing soln. of LR-DP
int opb[101];   // open Brackets
int clb[101];   // close Brackets
void generate(int l, int r) {
    if(l == r)  return;
    opb[l]++;
    generate(l, back[l][r]);
    generate(back[l][r] + 1, r);
    clb[r]++;
}
