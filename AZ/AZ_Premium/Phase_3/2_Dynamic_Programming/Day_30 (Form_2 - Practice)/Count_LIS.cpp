...
int dp[1001];
int rec(int i) {     // returns the length of LIS ending at i.
    // pruning 
...
ll dp1[1001];
ll rec1(int i) {     // returns the #LIS ending at i, if |LIS| ending at i = rec(i);
    // pruning
    if(i < 0)
        return 0;
    ...
    bool updated = 0;
    f(j, 0, i - 1) {
        if(arr[j] < arr[i] and rec(j) + 1 == rec(i)) {
            updated = 1;
            ans = (ans + rec1(j)) %P;
        }
    }
    if(ans == 0 and !updated) ans = 1;                           // ⭐⭐⭐
...
void solve()
{
    ...
    int mx_lis = 1;
    f(i, 0, n - 1)  mx_lis = max(mx_lis, rec(i));
