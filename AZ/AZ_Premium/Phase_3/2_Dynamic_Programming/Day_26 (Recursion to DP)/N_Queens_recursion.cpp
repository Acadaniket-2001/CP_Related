...
bool check(int row, int col) {
    f(i, 0, row - 1) {
        int prow = i;
        int pcol = queens[i];
        if(col == pcol or abs(col - pcol) == abs(row - prow))
            return 0;
    }
    return 1;
}

int rec(int lvl) {                  // ⭐⭐ returns the #ways to place queen in [lvl...n - 1] rows for a valid config.
    // level -> current row in which we are placing queen
