...
void find_LIS()
{
    ...
    f(i, 0, n - 1) { 
        ...
        else {                                                      // ⭐ appending arr[i] to: max_element < arr[i]
...
void print_LIS()
{
    ...
    f(i, 0, n - 1) { 
        if(lis.empty() or lis.back() < arr[i]) {
            lis.pb(arr[i]);
            insertedAt[i] = lis.size();                             // 🧠🧠 insertedAt[i] stores the best length at which arr[i] was inserted to form inc. subsequence. 
        }
        else {                                                      
            auto it = lower_bound(all(lis), arr[i]);
            *it = arr[i];
...
insertedAt[ ]:  1 2 3  4 4 3 4 5 2 3 
