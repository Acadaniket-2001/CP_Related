...
int dp[100100][8];                                                  // to track 3 bit number: (1 << 3) states are needed  
int rec(int lvl, int lastthree) {
    ...
    // Xition
    int ans = 0;
    if(lvl >= 3 and lastthree == 2) { // 010                        // ⭐ adding lvl >= 3 to differentiate b/w (010 and 10) lastthree states
        ans += rec(lvl + 1, 5);
    }
    else { //101
        ans += rec(lvl + 1, (((lastthree << 1) | 0) & 7));          // adding 0 towards end and clearing MSB
        ans += rec(lvl + 1, (((lastthree << 1) | 1) & 7));          // adding 1 towards end and clearing MSB
    }
...
int dp1[100100][5];
int rec1(int lvl, int state) {
    // pruning
    if(state == 4)  return 0;                      // y necessary ??? -> See OneNote AZ_Day_36_P3_Video_2

    // base case
    if(lvl == n)    return 1;

    // cache chk
    if(dp1[lvl][state] != -1)   return dp1[lvl][state];

    // Xition
    int ans = 0;
    if(state == 0) 
        ans = rec1(lvl + 1, 1) + rec1(lvl + 1, 0);
    else if(state == 1)
        ans = rec1(lvl + 1, 1) + rec1(lvl + 1, 2);
    else if(state == 2)
        ans = rec1(lvl + 1, 3) + rec1(lvl + 1, 0);
    else if(state == 3)
        ans = rec1(lvl + 1, 4) + rec1(lvl + 1, 2);

    // save and return
    return dp1[lvl][state] = ans;
}




void findSubstrUsingBitmask() {
    memset(dp, -1, sizeof dp);
    cout << rec(0, 0) << endl;
}

void findSubstrUsingAutomata() {
...
{ 
    cin >> n;
    findSubstrUsingBitmask();
    findSubstrUsingAutomata();
}
