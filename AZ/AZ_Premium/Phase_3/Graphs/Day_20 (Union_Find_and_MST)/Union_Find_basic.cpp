...
int find(int x) {
    if(par[x] == x) return x;
    else    return par[x] = find(par[x]);                // ⭐ path compression ... par[x] = .. wala part
}

void merge(int x ,int y) {
    int xroot = find(x), yroot = find(y);
    if(xroot != yroot) {
        if(ranks[xroot] >= ranks[yroot]) {               // ⭐ rank compression ... merging larger to smaller
            par[yroot] = xroot;
            ranks[xroot] += ranks[yroot];                // ⭐ rank[rootx] is updated and not rank[x] is updated
        }
        else {
            par[xroot] = yroot;
