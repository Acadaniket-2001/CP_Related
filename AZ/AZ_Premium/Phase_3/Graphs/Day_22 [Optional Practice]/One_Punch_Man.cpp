...
void bfs_01(state st) {
    ...
    while(!dq.empty()) {
        ...
        f(i, 0, 3) {
            ...
            if(is_valid(x, y)) {
                if(g[x][y] == '.') {
                    if(dis[x][y] > dis[node.ff][node.ss] + 0) {
                        dis[x][y] = dis[node.ff][node.ss] + 0;
                        dq.push_front({x, y});
                    }
                }
                else {
                    ...
                    f(ni, x - 1, x + 1) {                                      // 🤩⭐⭐🤩❤️❤️
                        f(nj, y - 1, y + 1) {
                            ...
                            if(node.ff == ni and node.ss == nj)    continue;

                            if(is_valid(ni, nj)) {
                                if(dis[ni][nj] > dis[node.ff][node.ss]  + 1) {
                                    dis[ni][nj] = dis[node.ff][node.ss]  + 1;
                                    dq.push_back({ni, nj});
                                }
                            }
                        }
                    ...
                    //             int yy = sty + nj;
                    //             if(is_valid(xx, yy)) {
                    //                 if(dis[xx][yy] > dis[node.ff][node.ss] + 1) {
                    //                     dis[xx][yy] = dis[node.ff][node.ss] + 1;
                    //                     dq.push_back({xx, yy});
                    //                 }
                    //             }
                    //         }
