...
vector<int> Bellman(vector<pair<int, pair<int, int>>> &edges, int st) {
    vector<int> dis = vector<int>(n + 1, 1e18);
...
void solve()
{
    ...
    vector<int> dis = Bellman(edges, 1);

    // just relax all edges one more time to detect -ve cycle;
    for(auto e: edges) {
        int u = e.ff;
        int v = e.ss.ff;
        int w = e.ss.ss;
    ...
    // just relax all edges |V| more times to detect all nodes reachable from nodes in -ve cycle;
    vector<int> dis1 = dis;
    f(i, 1, n) {
        for(auto e: edges) {
            int u = e.ff;
