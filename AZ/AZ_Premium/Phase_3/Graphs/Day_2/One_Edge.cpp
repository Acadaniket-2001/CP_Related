...
// Method 1 : Use dfs() to find the |component|

int n, m; 
vector<vector<int>> g;
vector<int> vis;

int size_dfs(int node) {
    vis[node] = 1;
...
void solve()
{
    ...
    ll ans = 0;
    f(i, 0, cnt_comp - 1) {
        ans += sizes[i] * (pre[cnt_comp - 1] - pre[i]);
    }
...
// Method 2 : Use cnt of each distinct color node in vis[ ] to find the |component|