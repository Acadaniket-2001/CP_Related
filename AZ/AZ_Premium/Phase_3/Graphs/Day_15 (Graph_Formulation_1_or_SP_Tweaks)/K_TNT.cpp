...
/*
Problem: Given K(TNT), find the SP from 'S' -> 'F'
...
using state = pair<pair<int, int>, int>;     // {{i, j}, left_tnt}
...
void bfs(state st) {
    dis.assign(n, vector<vector<int>>(m, vector<int>(k + 1, INF)));                      // ⭐ (k+1)=> [0, k]
    ...
    while(!q.empty()) {
        ...
        for(auto v: neighbours(node)) { 
            ...
            // if(dis[v.F.F][v.F.S][v.S] == INF) {                                              // also works, but only for BFS()
            if(dis[v.F.F][v.F.S][v.S] > dis[node.F.F][node.F.S][node.S] + 1) {            // ⭐ +1 is for edge cost = #cells reached
