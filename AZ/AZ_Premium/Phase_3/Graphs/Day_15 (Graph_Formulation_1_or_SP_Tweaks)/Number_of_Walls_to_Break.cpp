...
/*
Problem: Given a (N x M) grid. Find the minimum of walls needed to break to reach from 'S' -> 'F';
...
void bfs01(state st) {
    ...
    while(!dq.empty()) {
        ...
        for(auto v: neighbours(node)) {
            int wt = (arr[v.ff][v.ss] == '#');
            
            // if(dis[v.ff][v.ss] == INF) {                                       // ❌❌❌ works only for BFS()
            if(dis[v.ff][v.ss] > dis[node.ff][node.ss] + wt) {
                dis[v.ff][v.ss] = dis[node.ff][node.ss] + wt;
                if(wt)    dq.push_back(v);
...
void solve()
{
    ...
    bfs01(st);      // O(V + E) = O(n.m.k + 4.n.m.k) = O(N.M.K)
